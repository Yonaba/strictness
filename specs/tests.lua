...
test.assert_not_nil('defines a global named "global"', global)
test.assert_not_nil('defines a global named "globalize"', globalize)
test.assert_equal('global is a function', type(global), 'function')
...
test.assert_not_error('A newly assigned global takes the value nil by default', function() global ("n"); end)
...
test.assert_error('global() do not accept tables', function() global({}) end)
test.assert_error('global() do not accept numbers', function() global(1) end)
test.assert_error('global() do not accept nil', function() global(nil) end)
test.assert_error('global() do not accept boolean', function() global(true) end)
test.assert_error('global() do not accept functions', function() global(function() end) end)
test.assert_error('global() do not accept coroutine, userdata or threads', function() global(coroutine.create(function() end)) end)
test.assert_not_error('global() only accept strings', function() global("a") end)
test.assert_error('But not empty strings', function() global("") end)
test.assert_not_error('Multiple globals can be declared with global()', function() global("yy", "zz") end)
...
for _, varname in ipairs(reserved) do
  test.assert_error(('like %s'):format(varname), function() global(varname) end, 2)
end
...
print('  Varname "global" can be assigned to a global')
local glob, globz = _G.global, _G.globalize
test.assert_not_error('declared global "global"',function() global "global"; global = true end, 2)
test.assert_true('assigned var "global" a boolean value', global, 2)
test.assert_not_error('reassigned original global successfully', function() global = glob end, 2)

print('  Varname "globalize" can also be assigned to a global')
local globz = _G.globalize
test.assert_not_error('declared global "globalize"',function() global "globalize"; globalize = false end, 2)
test.assert_false('assigned var "globalize" a boolean value', globalize, 2)
test.assert_not_error('reassigned original globalize successfully', function() globalize = globz end, 2)

print(decorate('Functions declaring globals:'))
test.assert_error('Raises error when called normally', function() one = "one" end)
test.assert_not_error('globalize() wraps these functions so that they can write globals', globalize(function() one = "one" end))
...
test.assert_not_error('Let\' require strictness', function() require "strictness" end)
test.assert_not_error('We can now declare globals', function() global "var"; var = true end)
