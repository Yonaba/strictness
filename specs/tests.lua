...
test.assert_not_error('in case the original table had a metatable, it is restored', function() local mt = {}; local t = setmetatable(t,mt); strictness.strict(t); strictness.sloppy(t); assert(getmetatable(t) == mt) end)
test.assert_not_error('in case the original table had not a metatable, this is also resoected', function() local t = {} ; strictness.strict(t); strictness.sloppy(t); assert(getmetatable(t) == nil) end)
